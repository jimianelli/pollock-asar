...
in both traditional stock assessment models and in state-space versions (e.g.,
@stock2021). Based on our analysis, we confirm that estimates appear reasonable. However,
we caution about applying $\sigma_R$ estimates blindly in management settings. This is
because for EBS pollock, the SRR is heavily influenced by estimates of spawning biomass
being mainly larger than $B_{MSY}$ (to the right of $B_{MSY}$; far from the origin of the
SRR curve) instead of spawning biomass estimates that are much smaller than $B_{MSY}$
(near the origin, which would better inform estimates of steepness). We show that the SRR
is thus linked to the Tier 1 fishing mortality recommendation in two ways: 1) by the
assumption about $\sigma_R$ (smaller values increase the "precision" of $F_{MSY}$) and 2)
by having most of the data far from the origin means estimating steepness becomes less
certain and possibly unreliable. We provided an illustration of this in an external
simulation. In the 2023 assessment, $\sigma_R$ was specified at 1.0 as a precautionary
measure. We seek guidance on $\sigma_R$ approaches and general treatment of the SRR given
the direct management implications under Tier 1.
...
aspect by developing a parallel catch guideline that is intended to take into account the
role pollock plays in the ecosystem. We contrast this alternative guideline (in the form
of "advice") with historical patterns of catch. Additionally, we consider multi-species
mass-balance projections under scenarios where catch projections closer to the maximum
permissible are used. This was done using "ecosense" within the package Rpath (@rpath;
courtesy Andy Whitehouse).
...
-   Document the method used for determining the selectivity to use in the forward
    ...
    -   *We will be clearer about projection assumptions especially for selectivity. In
        this report we show the sensitivity of selectivity assumption and evaluated the
        sensitivity of Tier 1 ABC estimates given different selectivity estimates from
        historical annual values. In the next draft we will update how different
        selectivity assumptions perform based on an updated retrospective presentation for
...
revert to the prior as fewer years are included in the conditioning of the SRR curve. To
consider the variability of the SRR estimates due to additional years of data, we changed
the window of years used within the assessment model. Results showed given the prior and
other assumptions, the SRR estimates were relatively stable (@fig-yearlop2 and
@fig-trans).
...
#| for EBS pollock, 1978-2022. The clusters of black dots show resampled stock-recruit data."
#| label: fig-SRRsim
...
Traditionally the SRR form has been parameterized as a "Ricker" due to the presence of
cannibalism. As part of the review of different approaches for modeling the stock-recruit
relationship, we compared last year's model with different forms, priors, and periods
using the Beverton-Holt curve. The shorter and longer periods with the Beverton-Holt curve
(using the same priors) were similar to the Ricker curve, but had higher uncertainty
...
This figure also shows that the problem persists in Tier 2 along with a candidate hybrid
Tier 2 approach from the 2021 assessment (here labeled as "Tier 1.5")[^1] but Tier 3 is
insensitive. We note that the value of $\sigma_R$ affects the shape of the curve as well
(@fig-sigrplot1 and @fig-sigrplot2).

[^1]: This modification of Tier 2 simply applies the harmonic mean of $F_{MSY}$ instead of
    the arithmetic mean to make it slightly more precautionary.
...
to be known precisely. The resulting selectivity patterns are shown in @fig-selex. 
Results showed minor differences between the model with selectivity allowed
to be freely variable and the 2023 assessment (@fig-separable_ssb_r_vpa).
Results comparing the constrained ("separable") selectivity differed
substantially from last year's configuration (@fig-separable_ssb_r; see @fig-selex for
differences in selectivity patterns).

The large differences due to assuming separability also impacts the estimates of the stock
recruitment relationship (@fig-separable_srr). This figure also depicts the different
magnitude of the recent recruitment and the corresponding
increased uncertainty. In particular, the 2018
...
Results show that the SRR also varied slightly when a constant selectivity was assumed
(@fig-separable_srr).
...
selectivity estimates. The model has an option for which specific year to use for "future"
selectivity and for the number of years over which an average selection pattern could be
used.

Initial results showed that there could be large and variable impacts on the ABC estimates
under the current Tier 1 and for different Tiers (@fig-sel_ABC_plot). This was presumably
due to the fact that the selectivity changes can shift to younger or older ages in some
years. However, the SRR curve was insensitive to the selectivity estimate
(@fig-selsrrplot) because the future selectivity pattern is separate from the SRR
estimation.
...
modlst <- NULL
dftmp <- NULL
dftmp_path <- here::here("2024", "runs", "selsens_sept", "dftmp.rds")
modlst_path <- here::here("2024", "runs", "selsens_sept", "modlst.rds")
...
} else if (file.exists(dftmp_path) && file.exists(modlst_path)) {
  dftmp <- readRDS(dftmp_path)
  modlst <- readRDS(modlst_path)
} else {
  knitr::asis_output("**Note:** Selectivity sensitivity results were not found in `2024/runs/selsens_sept/`. Set `do_abc_sel <- 1` and re-render to regenerate these figures.")
}

if (!is.null(modlst) && length(modlst) > 0) {
...
# names(modlst)
# .OVERLAY=TRUE
if (!is.null(modlst) && length(modlst) >= 18) {
...
#| fig.cap: "Model results comparing last year's selected model (SRR 1978-2021)
#| with one where the
#| SRR was conditioned such that $F_{MSY}$ was equal to the SPR rate of $F_{35}$.
#| The vertical bars represent the 95% confidence intervals for the age-1 recruitment."
...
In past assessments we have mentioned that a commonly adopted approach 
is to use estimated natural mortality-at-age and year fro 
multispecies trophic interaction models (@trijoulet2020).
We developed an option to include the 2023 CEATTLE estimates of natural
mortality (@tbl-Mmatrix; @fig-Mmatrix_matrix). This resulted in slightly higher 
recruitment but lower spawning biomass in the near term (@fig-Mmatrix). 
This is due to the higher natural mortality for most ages and years compared 
to the base 2023 model.
...
#| messages: FALSE
#| label: tbl-Mmatrix
#| tbl-cap: "Natural mortality-at-age used over ages and time based on the CEATTLE 
#| 2023 model."
m_matrix <- as_tibble(M$M)[,1:10]
#dim(m_matrix)
names(m_matrix) <- 1:10
rownames(m_matrix) <- 1964:2023
...
p1 <- plot_recruitment(modlst[c(1, 6)], xlim = c(1963.5, 1980.5), alpha = .6,
  ...
  ggthemes::theme_few(base_size = 9) #+ scale_x_continuous(breaks=seq(1964, 1980, by = 4))
...
evaluation has begun but it more work is needed before it can be included in the
assessment.

# Further considerations of pollock and its role in the ecosystem
...
consistent with a specified MSY value. As an experiment, we conditioned the 
SRR curve to have the MSY value set to 1.75 and 1.3 million t. We then compared 
those curves with the 2023 model specifications
(@fig-srrplot_cond). When overplotted, the fit comparisons indicate somewhat worse fits to
the available years of data (post 1977; @fig-srrplot_cond2). However, the fits were
reasonable within the estimates of uncertainty. Here we conclude that the management
advice (under Tier 1) is sensitive to relatively small apparent perturbations in the shape
of the stock-recruitment curves. Such sensitivity is expected given the sections above.
Furthermore, we know that non-stationarity in the SRR is likely, especially given the
uncertainties of climate change. An alternative ABC-setting rule may be preferred that
stabilizes the advice while adhering to the NS1 guidelines.
...
cond_files <- c(
  here::here("2024", "runs", "condSRR_sept", "MSY175.rep"),
  here::here("2024", "runs", "condSRR_sept", "MSY130.rep")
)
if (all(file.exists(cond_files))) {
  srlst[[2]] <- read_rep(cond_files[1])
  srlst[[3]] <- read_rep(cond_files[2])
  ...
  missing_files <- basename(cond_files[!file.exists(cond_files)])
  names(srlst) <- "Last year"
  knitr::asis_output(paste0(
    "**Note:** Conditioned SRR outputs (",
    paste(missing_files, collapse = ", "),
    ") were not found in `2024/runs/condSRR_sept/`. Regenerate those runs to include the comparison figures."
  ))
  plot_srr(srlst[c(1)], alpha = .2, xlim = c(0, 6400), ylim = c(0, 85000), sizeout = 2, sizein = 4, yrsin = c(1977, 1979:2021)) + ggtitle("SRR as estimated in the 2023 assessment")
}
```
...
#| fig.cap: 'SRR curves as estimated in the 2023 assessment overlaid with those conditioned 
...
if (length(srlst) >= 3) {
  ...
  knitr::asis_output("**Note:** Conditioned SRR curves were omitted because the required outputs were not available.")
}
# plotly::ggplotly(p1)
```
...
In thinking about the role of pollock in the ecosystem and in relation to management
advice within the fisheries management plan (FMP) and Tier system, we considered pollock
as a key part of the forage base (say of 1-3 yr old pollock). A management goal with an
explicit consideration of ecosystem function might be to avoid low levels of forage base.
For example, if the forage base appeared to be close to say the lower 20th percentile from
historical estimates, then a management might include an adjustment that would occur then
to avoid any further declines (e.g., @fig-quant_prey).

While conceptually appealing, in practical terms including an explicit adjustment might be
difficult. For example, information on the abundance of those age classes would be
limited. The historical abundance of the prey base compared to spawning biomass shows that
the relationship between the two is poorly determined (@fig-ssb_prey). However, the forage
component does tend to decline at lower spawning biomass levels. This suggests that the
practice of conserving spawning biomass may be linked to downstream impacts of the main
forage ages.
...
pattern of pollock ABCs and catches and present a model for future catches that could be
more reflective of objectives related specifically to pollock within the ecosystem
(compared to TAC setting within the 2 million t cap constraint).
...
the coming year, $\bar{B}$ is the mean spawning biomass and $\lambda$ is a responsiveness
parameter (set to 0.5 in this example).

Results indicate that the combination of historical ABC and TAC decisions can be distilled
to a simple formula for providing catch advice (@fig-mp_hist). This advice is intended to
focus on the conservation goal of maintaining spawning biomass and provide ecosystem
stability--similar to the over-arching principle of the 2 Mt catch limit for the BSAI
region.

To evaluate performance of alternative pollock fishing at the ecosystem level, we
...
#| echo: FALSE
df <- NULL
test_rep <- here::here("2024", "runs", "test", "pm.rep")
if (file.exists(test_rep)) {
  M <- read_rep(test_rep)
  df <- data.frame(
    Year = 1964:2023,
    forage = rowSums(M$N),
  ...
  df |> ggplot(aes(x = Year, y = forage)) +
    ...
    geom_hline(yintercept = qfor[2], color = "blue", linetype = 2)
} else {
  knitr::asis_output("**Note:** The `2024/runs/test/pm.rep` output was not found, so forage-based plots are omitted.")
}
```
...
if (!is.null(df) && nrow(df) > 0) {
  ...
  knitr::asis_output("**Note:** Skipping the SSB versus forage panel because the forage time series was unavailable.")
}
# xlim(c(1980, 2024))
```
...
if (!is.null(df) && nrow(df) > 0) {
  ...
  knitr::asis_output("**Note:** Historical spawning biomass plot omitted because the forage data required for scaling were unavailable.")
}
```
...
#| message: FALSE
#| label: fig-mp_hist
#| fig.cap: "Results from applying a simple catch-advice rule given historical actual
#| catches (points, bottom panel) based on 
#| spawning biomass projections and mean SSB (top panel, blue and dashed line, respectively).
#| Arrows represent the catch advice given historical catches and different values of lambda."
df1 <- NULL
#for (resp in c(0.5))  {
if (exists("M")) {
  for (resp in c(0.05, 0.5, 0.95)) {
    for (i in 2000:2022) {
      ...
      lastidx <- i - 1977 + 14
      cat_mean <- mean(M$obs_catch[14:lastidx])
      TACy     <- M$obs_catch[lastidx]
      ssb_mean <- mean(M$SSB[14:lastidx, 2])
      ssb_this <- (M$SSB[lastidx, 2])
      ssb_next <- (M$SSB[lastidx + 1, 2])
      # cat_next   <- (ssb_next/ssb_this)^0.5 * cat_this
      A <- (ssb_next / ssb_mean)^resp * TACy
      delta <- A - TACy
      df1 <- data.frame(Year = i, 'Mean catch' = cat_mean, 'Actual catch' = TACy, 
                        'Catch advice' = A , 'Mean SSB' = ssb_mean, ssb_this = ssb_this, 
                        'SSB projected' = ssb_next, resp = resp, adj=(ssb_next/ssb_mean)^resp,
                        idx = lastidx, delta=delta
                        ) |> rbind(df1)
    }
  }
  p1 <- df1 |> mutate(TAC.Multiplier=adj, Lambda=as.factor(resp)) |> 
    ggplot(aes(x = Year, y = SSB.projected)) + ggthemes::theme_few() + ylim(c(0,NA)) +
    ylab("Projected SSB and mean")+
    geom_line(linewidth=1, color="purple")   + geom_line(aes(x=Year, y=Mean.SSB), color="green", linetype=2)
  p2 <- df1 |> mutate(Lambda=as.factor(resp)) |> 
    ggplot(aes(x = Year, y = Actual.catch, color=Lambda))  + ggthemes::theme_few() +
    geom_point(size=1.8) +
    geom_segment(aes(xend=Year+1,yend=Catch.advice), arrow=arrow(length = unit(0.04,"npc") )) + ylim(c(0,NA))+
    ylab("Catch (kt)")
  p1/p2
} else {
  knitr::asis_output("**Note:** Management procedure diagnostics were skipped because the test run outputs were unavailable.")
}
  #geom_segment(aes(xend=Year+1,yend=Catch.advice), arrow=arrow(length = unit(0.1,"cm"))) + ylim(c(0,NA))+
  #ylab("Catch (kt)")
    
#write_csv(df1,"adj.csv")
#df1 |> filter(resp==0.5) |> transmute(Year, TAC=Actual.catch, adj=sqrt(SSB.projected/Mean.SSB), A=delta) |> 
  #ggplot(aes(x=Year,y=A)) + geom_line()
#df1 |> pivot_longer(cols = c(3:5, 7,12), names_to = "var", values_to = "value") 
#p1 <- df1 |> pivot_longer(cols = c(3:5, 7), names_to = "var", values_to = "value") |> 
  #filter(resp==0.5) |> 
  #mutate(Year = ifelse(var %in% c("SSB.projected","Catch.advice"), Year + 1, Year)) |>
...
@monnahan2024. This involved performing multiple "no-U-turn sampler" (NUTS) chains and
note that that the potential scale reduction $\hat{R}$ was \<1.01 and the effective
...
hope to have this for future analysis to assess model fit. Another avenue that requires
more work is to include process error variances in the estimation--essentially apply 
...
simulated data were consistent with the observations. Here we attempt to expand on 
approaches that might be useful for evaluating models. We outline a summary of what
we found here in order to improve on our diagnostics for assessments.  After obtaining posterior
samples, let’s denote these samples as $\theta_1$, $\theta_2$, $\ldots$, $\theta_N$, where
N is the number of posterior samples. The next step is to generate predictive samples. For each
...
is true, and 0 otherwise. This gives the count of predictive samples that are less than or
...
predictive distribution. This can include comparing the observed data’s summary
statistics (e.g., mean, variance) relative to those of the posterior predictive
distribution.

Updating the 2023 model for MCMC runs using ADNUTS, we were able to achieve reasonable
...
and @fig-bayesdiag2 shows relationships between the slowest mixing parameters. The result
from sampling showed that for the 1340 parameters, there were 1,000 iterations over 8
...
#| For interpretation and guidance on these panels please see @carpenter2017stan. "
...
#| fig.cap: "Joint posterior plots for the slowest mixing parameters:
#| The red dots and elipses indicate point estimate and covariance of the 
#| MLE results. Trace plots are shown
#| on the diagonals. Upper off-diagonals show correlation coefficients between
#| parameters. "
...
if (loadup){
  ...
  compares_path <- here::here("compares.Rdata")
  if (file.exists(compares_path)) {
    load(compares_path)
    has_compares <- TRUE
  }
}
...
alternative presentation shows the differences by age over time (@fig-selmainage). These
...
if (isTRUE(has_compares) && exists("Plot_Sel")) {
  p<-Plot_Sel() + ggthemes::theme_few(base_size=9)
  p
} else {
  knitr::asis_output("**Note:** Platform comparison plots are unavailable because `compares.Rdata` was not found. Re-run the supporting models and set `loadup <- TRUE` to regenerate them.")
}
```
...
#| modeling platforms."
if (isTRUE(has_compares) && exists("Plot_Sel_age")) {
  p <- Plot_Sel_age()
  plotly::ggplotly(p)
} else {
  knitr::asis_output("**Note:** Selectivity-at-age comparisons are omitted because the precomputed platform outputs were unavailable.")
}
```
...
#| modeling platforms."
if (isTRUE(has_compares) && exists("Plot_SSB")) {
  p<- Plot_SSB()
  plotly::ggplotly(p)
} else {
  knitr::asis_output("**Note:** Platform-level spawning biomass comparisons are omitted because `compares.Rdata` was not available.")
}
```
...
#| platforms."
if (isTRUE(has_compares) && exists("Plot_SRR")) {
  p<- Plot_SRR()
  p
} else {
  knitr::asis_output("**Note:** Stock-recruitment comparisons across platforms are omitted because the supporting results were not available.")
}
```

## Additional AMAK runs

In an attempt to get different platforms closer to each other, some tuning of the model
specification for AMAK was undertaken. This included some comparisons with the early CPUE
data added, and with a newly developed 3-parameter double logistic parameterization:
...
am_ts <- NULL
am_fit <- NULL
if (isTRUE(has_compares) &&
    all(c("pm_run", "pm_run2", "am_run", "am_run2", "am_run3") %in% ls())) {
  ...
  knitr::asis_output("**Note:** AMAK comparison plots are unavailable because the supporting objects were not loaded.")
}
```
...
if (!is.null(am_sel)) {
  ...
  knitr::asis_output("**Note:** Selectivity-at-age comparisons within AMAK are omitted because the supporting runs were unavailable.")
}
```
...
if (!is.null(am_ts)) {
  ...
  knitr::asis_output("**Note:** AMAK SSB comparisons are omitted because the supporting runs were unavailable.")
}
```
...
if (!is.null(am_fit)) {
  ...
  knitr::asis_output("**Note:** AMAK index-fit comparisons are omitted because the supporting runs were unavailable.")
}
```
...
#| fig.cap: "Comparison of the fit to indices between different modeling 
#| specifications using the AMAK software platform."
if (exists("am_run3") && !is.null(am_run3$fit)) {
  Plot_index(df=am_run3$fit, idx=c(1,2,5,6)) + ggthemes::theme_few(base_size=9)
} else {
  knitr::asis_output("**Note:** The AMAK CPUE fit comparison was skipped because the necessary run outputs were unavailable.")
}
```
...
if (!is.null(am_ts)) {
  ...
  knitr::asis_output("**Note:** AMAK SRR comparisons were skipped because the supporting runs were unavailable.")
}
```
...
```{r echo=FALSE}
if (isTRUE(has_compares) && all(c("r1","r2","r3","r4") %in% ls())) {
  ss_sel <- rbind(r1$sel,r2$sel,r3$sel,r4$sel)
  p1 <- Plot_Sel(ss_sel)
  p1
} else {
  knitr::asis_output("**Note:** SS3 comparison plots were skipped because the required Stock Synthesis runs were not loaded.")
}
```
...
if (exists("ss_sel")) {
  ...
  knitr::asis_output("**Note:** SS3 selectivity-at-age comparisons were skipped because the required runs were not loaded.")
}
```
...
```{r echo=FALSE}
if (isTRUE(has_compares) && all(c("r1","r2","r3","r4") %in% ls())) {
  ss_ts  <- rbind(r1$ts,r2$ts,r3$ts,r4$ts)
  p1 <- Plot_SSB(ss_ts)
  ggplotly(p1)
} else {
  knitr::asis_output("**Note:** SS3 biomass comparisons were skipped because the required Stock Synthesis runs were not loaded.")
}
```
